...
def _decode_click_url(href: str) -> str:
    ...
    try:
        ...
        dest = qs.get("dest", [""])[0]
        if dest:
            return base64.urlsafe_b64decode(dest + "==").decode(
...
def parse_web_results(soup: BeautifulSoup) -> list[WebResult]:
    ...
    for article in soup.find_all(
        "article", class_=lambda c: bool(c and "animate-fade-in" in c and "pb-6" in c)
    ):
        ...
        url = _decode_click_url(str(title_a.get("href", "")))
        if not url and cite:
            url = str(cite.get("title", cite.get_text(strip=True)))

        sitelinks: list[dict[str, str]] = []
        for a in article.select("div.mt-3 a"):
            sitelinks.append({"text": a.get_text(strip=True), "url": str(a.get("href", ""))})
        ...
        results.append(
            WebResult(
                ...
                favicon_url=str(favicon.get("src", "")) if favicon else "",
                sitelinks=sitelinks,
            )
        )
    return results
...
def parse_news_results(soup: BeautifulSoup) -> list[NewsResult]:
    ...
    for article in soup.find_all("article", class_="h-full"):
        ...
        results.append(
            NewsResult(
                title=str(link.get("aria-label", ""))
                or (title_p.get_text(strip=True) if title_p else ""),
                url=str(link.get("href", "")),
                source=source.get_text(strip=True) if source else "",
                published_at=time_span.get_text(strip=True) if time_span else "",
                thumbnail=str(thumbnail.get("src", "")) if thumbnail else "",
            )
        )
    return results
    ...
    for li in soup.find_all("li", attrs={"data-testid": "shopping-card"}):
        ...
        results.append(
            ShoppingResult(
                ...
                buy_url=str(buy_a.get("href", "")) if buy_a else "",
            )
        )
    return results
...
def parse_image_results(soup: BeautifulSoup) -> list[ImageResult]:
    ...
    for article in soup.find_all("article", attrs={"role": "button"}):
        label = str(article.get("aria-label", ""))
...
def parse_pagination(soup: BeautifulSoup) -> list[int]:
    ...
    nav = soup.find("nav", attrs={"aria-label": lambda v: bool(v and "صفحه" in v)})
    if not nav:
        return []
    pages = []
