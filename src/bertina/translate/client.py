...
def _parse_response(
    ...
    try:
        ...
        raise BertinaParseError(f"missing translatedText in response: {data}") from exc
    raw_detected = data.get("detectedLang")
    detected: Language | None = None
    if raw_detected:
        try:
            detected = Language(raw_detected)
        except ValueError:
            logger.debug("unknown detectedLang %r", raw_detected)
    ...
    def translate(
        ...
        *,
        ...
        try:
            ...
            raise BertinaParseError(f"invalid JSON response: {exc}") from exc
        return _parse_response(data, text, source, target)
