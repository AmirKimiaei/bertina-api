...
from .._parsers import _safe_select
...
def _parse_card(article: Tag) -> RadarItem | None:
    try:
        ...
        rank = (
            int(rank_text.translate(str.maketrans("۰۱۲۳۴۵۶۷۸۹", "0123456789")))
            if rank_text
            else 0
        )

        category_btn = article.find(
            attrs={"data-testid": lambda v: bool(v and "radar-card-category-" in v)}
        )
        subtopic_a = article.find(
            attrs={"data-testid": lambda v: bool(v and "radar-card-subtopic-" in v)}
        )
        title_h3 = _safe_select(article, "h3")
        summary_p = _safe_select(article, "p.whitespace-pre-line", "p.line-clamp-6")

        # Entities (tag pills)
        entities_ul = article.find(
            attrs={"data-testid": lambda v: bool(v and "radar-card-entities-" in v)}
        )
        entities: list[str] = []
        if entities_ul:
            entities = [
                a.get_text(strip=True)
                for a in entities_ul.find_all("a")
                if a.get_text(strip=True)
            ]

        # Source name
        source_span = article.find(
            attrs={"data-testid": lambda v: bool(v and "radar-card-source-" in v)}
        )
        source = ""
        if source_span:
            font_span = source_span.find(
                "span", class_=lambda c: c and "font-medium" in c
            )
            source = font_span.get_text(strip=True) if font_span else ""

        # Time
        time_span = article.find(
            attrs={"data-testid": lambda v: bool(v and "radar-card-time-" in v)}
        )

        # Thumbnail
        thumb_a = article.find(
            attrs={"data-testid": lambda v: bool(v and "radar-card-thumb-" in v)}
        )
        thumbnail = ""
        if thumb_a:
            img = thumb_a.find("img")
            thumbnail = str(img.get("src", "")) if img else ""

        # News search URL
        news_a = article.find(
            attrs={"data-testid": lambda v: bool(v and "radar-card-news-link-" in v)}
        )
        ...
        return RadarItem(
            ...
            thumbnail=thumbnail,
            entities=entities,
            news_url=str(news_a.get("href", "")) if news_a else "",
        )
...
def parse_radar_page(soup: BeautifulSoup) -> RadarPage:
    sections = soup.find_all("section", class_=lambda c: bool(c and "space-y-4" in c))
    ...
    for section in sections:
        ...
        cards = section.find_all(
            "article", attrs={"data-testid": lambda v: bool(v and "radar-card-" in v)}
        )
    ...
    last_updated = ""
    for tag in soup.find_all(string=lambda t: bool(t and "آخرین بروزرسانی" in t)):
        parent = tag.parent
        if parent is None:
            continue
        gp = parent.parent
